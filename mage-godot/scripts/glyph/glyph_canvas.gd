...
var min_point_spacing: float = 4.0
...
func add_point(point: Vector2, force: bool = false) -> bool:
	if not _stroke_open:
		return false
...
func take_strokes() -> Array[PackedVector2Array]:
	var finished: Array[PackedVector2Array] = strokes
	strokes = []
	_current = PackedVector2Array()
	_stroke_open = false
	return finished
