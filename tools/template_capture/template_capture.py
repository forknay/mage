...
as a raw Point, appended onto the current Stroke -- exactly like
test_canvas.py does -- so the *points* come straight from your mouse
input, not from the saved image. The PNG snapshot saved alongside each
template is purely a human-readable reference; it is never read back in
for recognition.
...
from merge_intersecting_strokes import DEFAULT_TOUCH_THRESHOLD, count_touch_units
...
class TemplateCaptureTool:
    def __init__(self, templates_dir: str = DEFAULT_TEMPLATES_DIR, images_dir: str = DEFAULT_TEMPLATE_IMAGES_DIR):
        ...
        # One entry per physically-separate pen-stroke drawn so far this
        # session; a new Stroke is appended on every mouse-down, and points
        # accumulate onto whichever Stroke is currently last while dragging.
        self.current_strokes: list[Stroke] = []
    ...
    def draw_event(self, event, x, y, flags, param):
        ...
        mouse-down/drag/up is recorded as a Point onto the active Stroke,
        the same way test_canvas.py collects a gesture to recognize."""
        if event == cv2.EVENT_LBUTTONDOWN:
            self.drawing = True
            self.last_point = (x, y)
            self.current_strokes.append(Stroke(points=[Point(float(x), float(y))]))
            ...
            if self.drawing and self.last_point is not None:
                ...
                self.last_point = (x, y)
                self.current_strokes[-1].points.append(Point(float(x), float(y)))

        elif event == cv2.EVENT_LBUTTONUP:
            self.drawing = False
            self.last_point = None
            # Next EVENT_LBUTTONDOWN starts a brand new Stroke -- there's no
            # counter to bump here anymore, the next `append` above does it.
    ...
    def save_current(self):
        total_points = sum(len(s.points) for s in self.current_strokes)
        if total_points < 2:
            print("Nothing to save yet -- draw something first.")
            return

        print(f"\nCaptured {total_points} points across {len(self.current_strokes)} stroke(s).")
        ...
        template_path = append_template(
            name=name,
            level=level,
            strokes=list(self.current_strokes),
    ...
    def _prompt_level(self) -> int:
        ...
        """
        actual_units = count_touch_units(self.current_strokes, DEFAULT_TOUCH_THRESHOLD)
    ...
    def _save_snapshot(self, name: str) -> str:
        ...
        never read back in; the recognizer only ever uses `strokes`.
        """
        slug = slugify(name)
