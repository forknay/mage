...
#
# THE MAIN "HOW PERMISSIVE IS RECOGNITION OVERALL" KNOB. Lower ->
# sloppier/noisier drawings still get accepted. This alone does NOT erode
# the big penalty a genuinely missing/extra stroke gets (see section 4
# below): that penalty is steep enough that a real structural mismatch's
# score sits far below even a low min_score, while it only takes a modest
# min_score drop to start accepting ordinary hand-drawn noise.
...
# Part of the "overall permissiveness" knobs -- lower it further to be more
# forgiving of gestures drawn at a slightly different proportion than the
# template; it has no bearing on the missing/extra-stroke penalty below.
...
# without also forgiving a structurally different shape:
#
#   - CLOUD_DISTANCE_PENALTY_THRESHOLD is the size of the "this is just
#     noise" zone. Widening it makes MORE of a sloppy stroke's per-point
#     error go completely unpunished -- raise this for permissiveness.
#   - CLOUD_DISTANCE_EXPONENT is how hard a distance gets punished once it
#     DOES exceed that zone. This is the "big penalty" knob: keep it >= 2
#     (or raise it further) so that a genuinely missing/extra stroke -- whose
#     points sit far outside the noise zone -- still craters the score even
#     as the noise zone itself gets more generous.
#   - CLOUD_DISTANCE_MAX_WEIGHT controls how much a single worst-offending
#     point can tank the whole score by itself. Lowering it makes the score
#     more resistant to one noisy point (permissive), without weakening the
#     missing/extra-stroke penalty, which comes from MANY points being
#     wrong at once (a whole stroke's worth), not from one outlier -- the
#     average term alone still catches that.
#
# All four are also exposed as QRecognizer constructor arguments
# (cloud_distance_penalty_threshold / _exponent / _max_weight), so a caller
# can override them per-instance instead of editing this file, in addition
# to these being the defaults used when they don't.
...
CLOUD_DISTANCE_PENALTY_THRESHOLD: float = 30.0
...
# punished more severely relative to small, everywhere-present noise. Raised
# alongside the wider threshold above specifically so a real structural
# mismatch (a whole missing/extra stroke's worth of unmatched points, each
# far past the threshold) still collapses the score to ~0, even though more
# ordinary noise now goes unpunished by the wider threshold itself.
CLOUD_DISTANCE_EXPONENT: float = 2.5
...
# worst point). Lowered so one single noisy/outlier point doesn't dominate
# the score; a genuinely missing/extra stroke still tanks the score because
# it drags the AVERAGE down too (many points wrong at once), not just the
# max.
CLOUD_DISTANCE_MAX_WEIGHT: float = 0.25
...
# built-in fallback. Kept a bit above DEFAULT_MIN_SCORE by default -- a
# freshly captured template is usually drawn once, carefully, so there's
# less reason to pre-forgive it as much as ad-hoc recognition input.
DEFAULT_CAPTURE_MIN_SCORE: float = 0.5
...
# does not affect the raw (x, y) points recorded for recognition.
BRUSH_THICKNESS: int = 4

# Cycle of BGR colors used to draw bounding boxes/labels around each
# recognized Feature in test_canvas.py, one color per feature index
# (wrapping around via modulo once there are more features than colors).
FEATURE_COLORS = [
    (0, 255, 0),
    (0, 128, 255),
    (255, 200, 0),
...
# =============================================================================
# A "spell" is a relational layout of already-recognized features (see
...
# Individual SpellFeatureSlots can override this value.
...
# sector -- a feature sitting essentially AT the spell's center doesn't
